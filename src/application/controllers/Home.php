...
class Home extends Management_Controller
{
		...
		if ($this->input->method(FALSE) == 'post') {
			...
			if ($this->form_validation->run() != FALSE) {
				...
				if ($is_valid) {
					...
					// lakukan upload file foto pengunjung ke storage
					$data_foto = base64_decode($data_foto);
					$file_path = "//tmp//" . date('YmdHis') . $data['nik'] . '.jpg';
					file_put_contents($file_path, $data_foto);

					// generate uuid
					$uuid       = uniqid() . uniqid() . rand(0, 9999);
					$image_url  = $this->_upload_to_file_storage($file_path, $uuid . '.jpg');

					if ($image_url['STATUS']) {
						$data['foto'] = $image_url['URL'];
					} else {
						$data['foto'] = null;
					}
	...
	// fungsi untuk upload ke file storage
	function _upload_to_file_storage($file_path = null, $file_name = null)
	{
		$output = [
			'STATUS'  => false,
			'URL'     => null,
		...
		// masukkan dalam data post untuk upload ke server
		$p_bucket     = 'visitors';
		$p_path       = 'guests';
		$p_file       = new CURLFILE($file_path);
		$p_file->setPostFilename($file_name);
		$p_overwrite  = true;
		...
		$ch = curl_init(API_UPLOAD_URL . 'fileops/upload');
		...
		if ($resp && $json = json_decode($resp, true)) {
			...
			$output['STATUS']   = false;
			$output['MESSAGE']  = 'Could not upload document file to server.';
		}
